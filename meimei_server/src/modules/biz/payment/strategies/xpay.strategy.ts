...
export class XPayStrategy implements PaymentStrategy {
  ...
  constructor(private readonly httpService: HttpService) {
    ...
    this.channelConfig = {
      ...
      platformKey: 'pk_test_abcd1234efgh5678',
      platformSecret: 'sk_test_secret_key_here',
  ...
  async createPayment(params: CreatePaymentParams): Promise<CreatePaymentResult> {
    this.logger.log(`X支付创建订单: ${params.orderNo}`);
    ...
    try {
      ...
      this.logger.debug(`请求参数: ${JSON.stringify(requestData)}`);
      ...
      this.logger.error(`X支付创建订单异常: ${error.message}`, error.stack);
      return {
        success: false,
        paymentNo: params.orderNo,
  ...
  async queryPayment(paymentNo: string): Promise<QueryPaymentResult> {
    this.logger.log(`X支付查询订单: ${paymentNo}`);
    ...
    try {
      ...
      this.logger.error(`X支付查询订单异常: ${error.message}`, error.stack);
      return {
        success: false,
        status: PaymentStatus.FAILED,
  ...
  async handleNotify(data: any): Promise<NotifyResult> {
    this.logger.log(`X支付处理回调: ${data.order_no}`);
