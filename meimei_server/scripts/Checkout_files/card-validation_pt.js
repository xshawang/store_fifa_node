...
(function() {
  ...
  if (cardExpiryInput) {
    ...
    cardExpiryInput.addEventListener('blur', function() {
      ...
      if (cleanValue.length !== 4) {
        showError(this, 'Formato inválido. Use MM/YY');
        return;
      }
      ...
      // Validate month
      if (month < 1 || month > 12) {
        showError(this, 'Mês inválido. Deve estar entre 01 e 12.');
        return;
      }
      ...
      if (year < currentYear || (year === currentYear && month < currentMonth)) {
        showError(this, 'Cartão expirado.');
        return;
      }
  ...
  if (cardCvvInput) {
    ...
    cardCvvInput.addEventListener('blur', function() {
      ...
      if (cvv.length !== expectedLength) {
        showError(this, `O CVV deve ter ${expectedLength} dígitos`);
        return;
      }
  ...
  if (cardNameInput) {
    ...
    cardNameInput.addEventListener('blur', function() {
      ...
      if (name.length === 0) {
        showError(this, 'O nome do titular do cartão é obrigatório');
        return;
      }
      
      if (name.length < 2) {
        showError(this, 'Nome muito curto');
        return;
      }
  ...
  addressFields.forEach(({ input, name }) => {
    if (input) {
      ...
      input.addEventListener('blur', function() {
        ...
        //     showError(this, 'Formato de número de telefone inválido');
        //     return;
        //   }
        ...
        if (input === postalCodeInput) {
          if (value.length < 3) {
            showError(this, 'Código postal muito curto');
            return;
          }
        }
    ...
    payButton.addEventListener('click', function(e) {
      ...
      [firstNameInput, lastNameInput, addressInput, postalCodeInput, phoneInput].forEach(input => {
        if (input) {
          ...
          if (!value) {
            const fieldName = input.id === 'TextField0' ? 'O nome' :
                            input.id === 'TextField1' ? 'O sobrenome' :
                            input.id === 'shipping-address1' ? 'O endereço' :
                            input.id === 'TextField3' ? 'O código postal' : 'O telefone';
            showError(input, `${fieldName} é obrigatório`);
            isValid = false;
